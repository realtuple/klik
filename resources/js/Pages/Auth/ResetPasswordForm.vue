...
<template>
    <AuthTemplate>
        <div
            ...
            <Form class="text-center" :action="route('auth.resetPassword')" method="post" #default="{ processing }">
                ...
                <p v-if="usePage().props.errors.email" class="mt-1 text-red-400"> {{ usePage().props.errors.email[0] }}
                </p>
                ...
                <p v-if="usePage().props.errors.password" class="mt-1 text-red-400"> {{
                    usePage().props.errors.password[0] }}</p>
                <Input name="password_confirmation" type="password" placeholder="Repeat Password" minlength="8"
                    class="mt-2" required />
                <p v-if="usePage().props.errors.password_confirmation" class="mt-1 text-red-400"> {{
                    usePage().props.errors.password[0] }}</p>
                <Button type="submit" class="mt-5 w-[80%]" :disabled=processing> {{ processing ? "Processing..." : "Log in" }}</Button><br /><br />
            </Form>
        </div>
    </AuthTemplate>
